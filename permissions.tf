...
locals {
  ...
  has_any_secrets = (
    // Terraform's `count`-parameter cannot read properties only visible after apply
    length(var.application_container.secrets_from_ssm) > 0 ||
    length(var.application_container.secrets_from_secretsmanager) > 0 ||
    length(var.application_container.secrets) > 0 ||
    length(var.application_container.secrets_to_override) > 0
  )
}
